...
export default function Message({ text, type, detectedLang, summary, translation, isLoading, onSummarize }) {
  return (
    <motion.div
      ...
      className={`p-4 rounded-xl max-w-[80%] ${type === 'user' ? 'bg-blue-600 text-white self-end' : 'bg-gray-800 text-gray-200 self-start'}`}
    >
      <p>{text}</p>
      ...
      ) : detectedLang === 'en' && !isLoading.summarizing ? (
        <button onClick={onSummarize} className="mt-2 px-3 py-1 bg-green-500 text-white rounded-md text-sm hover:bg-green-600 transition">
          Summarize
        </button>
      ) : null}
    </motion.div>
